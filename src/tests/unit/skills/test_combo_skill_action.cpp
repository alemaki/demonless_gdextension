...
TEST_SUITE("TestComboSkillAction")
{
    ...
    TEST_CASE_FIXTURE(ComboSkillActionFixture, "Chains trough all stages")
    {
        combo->step(0.6);
        CHECK(combo->try_buffer_next_action());
    ...
    TEST_CASE_FIXTURE(ComboSkillActionFixture, "Combo emits action_changed signal when switching actions")
    {
        ::SignalObserver::watch_signals(combo);
        combo->step(0.6);
        CHECK(combo->try_buffer_next_action());
        ...
        CHECK_EQ(::SignalObserver::get_signal_emitted_count(combo, "action_changed"), 1);
        combo->step(0.1);
        CHECK_EQ(::SignalObserver::get_signal_emitted_count(combo, "action_changed"), 2);

        combo->step(1.0); /* finish up */
        CHECK_EQ(::SignalObserver::get_signal_emitted_count(combo, "action_changed"), 2);
    }

    TEST_CASE_FIXTURE(ComboSkillActionFixture, "get_animation returns animation of current skill")
    {
        stage1->set_animation("1");
        stage2->set_animation("2");
        stage3->set_animation("3");
        ...
        CHECK_EQ(combo->get_animation(), godot::StringName("2"));
        combo->step(0.1);
        CHECK_EQ(combo->get_animation(), godot::StringName("3"));
