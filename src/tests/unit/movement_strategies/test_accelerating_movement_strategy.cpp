...
TEST_SUITE("AcceleratingMovementStrategy Full Acceleration Tests")
{
    ...
    TEST_CASE_FIXTURE(AcceleratingFixture, "Displacement correct when hitting min_speed mid-frame")
    {
        // Start close to min_speed so it hits mid-frame
        ctx->set_speed(3.0);
        strat->set_acceleration_per_second(-4.0);
        strat->set_min_speed(2.0);
        strat->set_max_speed(10.0);
        ...
        double time_decelerating = (3.0 - 2.0) / 4.0;
        double time_at_min = delta - time_decelerating;
        double displacement = 3.0 * time_decelerating - 0.5 * 4.0 * time_decelerating * time_decelerating; // during deceleration
        displacement += 2.0 * time_at_min;
