...
BTTask::Status BTExecuteAttack::_tick(double delta)
{
    TASK_FAIL_COND_COMP_MSG(
        ...
        vformat("Blackboard has no value named: %s", this->attack_to_execute)
    );

    TASK_FAIL_COND_COMP_MSG(!(this->get_blackboard()->has_var("target")), " No target in blackboard");
    ...
    TASK_FAIL_COND_COMP_MSG(attack == nullptr, vformat("\"%s\" is not Attack as expected in blackboard", this->attack_to_execute));
...
void BTExecuteAttack::_enter()
{
    ...
    ERR_FAIL_COND_MSG(attack == nullptr, vformat("%s is not SkillAction as expected", this->attack_to_execute));
    attack->reset();
