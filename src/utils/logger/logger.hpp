#ifndef SRC_LOGGER_HPP
#define SRC_LOGGER_HPP

#include <godot_cpp/variant/string.hpp>
#include <godot_cpp/variant/utility_functions.hpp>
#include <godot_cpp/templates/hash_set.hpp>

#include "behaviour_tree/utils/macros.hpp"
#include "behaviour_tree/utils/utils.hpp"

namespace utils
{
    class Logger
    {
    private:
        Logger(){};
        Logger(const Logger&) = delete;

    public:
        static Logger* instance;

        enum Level
        {
            CRIT_ERR = 0, /* CRITICALS and ERRORS are handled in place with godot's CRASH_ERR and ERR_PTINT- always enabled */
            INFO = 1,
            DEBG = 2,
        };

        int enabled  = CRIT_ERR | INFO | DEBG;
        bool every_class_enabled = true;

        godot::HashSet<godot::StringName> enabled_classes = {};
        godot::HashSet<godot::StringName> disabled_classes = {};

        CREATE_GETTER_SETTER_BOOL_DEFAULT(every_class_enabled);

        inline bool is_class_enabled(const godot::StringName& name) const
        {
            if ((this->every_class_enabled) && (!disabled_classes.has(name)))
            {
                return true;
            }
            else if ((!this->every_class_enabled) && (enabled_classes.has(name)))
            {
                return true;
            }
            return false;
        }
        void disable_class(const godot::StringName& name);
        void enable_class(const godot::StringName& name);

        void enable_level(Level level);
        void disable_level(Level level);
        void enable_all_levels();

        template <typename T>
        inline void log_info(godot::String message)
        {
            if (((this->enabled & Level::INFO) == 0) || (!this->is_class_enabled(T::get_class_static()))) 
            {
                return;
            }
            godot::UtilityFunctions::print("[INFO]", "[", T::get_class_static(), "]", message);
        }

        template <typename T>
        inline void log_debug(godot::String message)
        {
            if ((this->enabled & Level::DEBG) == 0 || (!this->is_class_enabled(T::get_class_static()))) 
            {
                return;
            }
            godot::UtilityFunctions::print("[DEBG]", "[", T::get_class_static(), "]", message);
        }
    };

    inline Logger* Logger::instance = new Logger();

} // namespace utils

#endif /* SRC_LOGGER_HPP */
